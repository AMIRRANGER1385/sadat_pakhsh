...
function handle_action(): never {
 ...
 case 'delete_product':query('UPDATE ns_products SET active=0,version=version+1 WHERE id=?',[number_input('id',1,PHP_INT_MAX)]);break;
 ...
 case 'order':
  ...
  transaction(function()use($id,$status){$o=one('SELECT * FROM ns_orders WHERE id=? FOR UPDATE',[$id]);if(!$o)throw new ShopError('سفارش یافت نشد.');$allowed=['PAID'=>['SHIPPED'],'SHIPPED'=>['DELIVERED'],'PENDING'=>['CANCELLED']];if(!in_array($status,$allowed[$o['status']]??[],true))throw new ShopError('تغییر وضعیت مجاز نیست.');if($status==='CANCELLED'&&$o['authority'])throw new ShopError('سفارش دارای authority باید ابتدا در درگاه بررسی شود.');query('UPDATE ns_orders SET status=? WHERE id=?',[$status,$id]);if($status==='CANCELLED')foreach(all('SELECT * FROM ns_order_items WHERE order_id=?',[$id])as$i)query('UPDATE ns_products SET stock=stock+?,version=version+1 WHERE id=?',[$i['quantity'],$i['product_id']]);});break;
