...
function guide_page(string $slug): bool {
 ...
 ?><div class="container page-content guide-layout"><article class="panel guide-article"><nav class="breadcrumbs" aria-label="مسیر صفحه"><a href="/">خانه</a> / <a href="/guides">راهنمای خرید</a></nav><h1><?=h($guide['title'])?></h1><p class="guide-byline">منتشرکننده: <a href="/about">نایلکس سادات</a> · آخرین ویرایش: <time datetime="<?=h($guide['date_modified'])?>"><?=h($guide['date_modified'])?></time></p><p class="seo-intro"><?=h($guide['intro'])?></p><nav class="guide-toc" aria-label="فهرست مقاله"><strong>در این راهنما می‌خوانید</strong><ol><?php foreach($guide['sections'] as $i=>$section):?><li><a href="#section-<?=$i+1?>"><?=h($section['heading'])?></a></li><?php endforeach;?></ol></nav><?php foreach($guide['sections'] as $i=>$section):?><section id="section-<?=$i+1?>"><h2><?=h($section['heading'])?></h2><?php foreach($section['paragraphs'] as $paragraph):?><p><?=h($paragraph)?></p><?php endforeach;if(!empty($section['list'])):?><ul><?php foreach($section['list'] as $item):?><li><?=h($item)?></li><?php endforeach;?></ul><?php endif;?></section><?php endforeach;visible_faq($guide['faq'],'پرسش‌های رایج');if($guide['sources']):?><section class="guide-sources"><h2>منابع و مطالعه بیشتر</h2><ul><?php foreach($guide['sources'] as $source):?><li><a href="<?=h($source['url'])?>" rel="external"><?=h($source['title'])?></a></li><?php endforeach;?></ul></section><?php endif;?></article><aside class="guide-shopping panel"><h2>انتخاب محصول مرتبط</h2><p>قیمت، موجودی و حداقل سفارش را در دسته موردنظر بررسی کنید.</p><ul><?php foreach(seo_categories() as $category):if(!in_array(category_profile($category)['key'],$guide['category_keys'],true))continue;?><li><a href="<?=h(category_path($category))?>"><?=h($category['name'])?></a></li><?php endforeach;?></ul><a class="text-link" href="/contact">مشاوره برای انتخاب و سفارش</a><a class="text-link" href="/wholesale">درخواست حساب عمده</a></aside><?php $related=[];foreach($guide['related_slugs'] as $relatedSlug)if(isset($guides[$relatedSlug])&&$relatedSlug!==$slug)$related[]=$guides[$relatedSlug];guide_cards($related,'راهنماهای مرتبط');?></div><?php page_footer();return true;
}
