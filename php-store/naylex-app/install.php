...
if($_SERVER['REQUEST_METHOD']==='POST'){
 ...
 try {
  ...
  transaction(function()use($username,$password){
   $names=['کیسه فریزر','کیسه زباله','نایلکس','نایلون','بسته‌بندی','سفره یکبار مصرف'];$ids=[];
   foreach($names as $i=>$name){query('INSERT INTO ns_categories(name,slug,sort_order) VALUES (?,?,?) ON DUPLICATE KEY UPDATE name=VALUES(name)',[$name,['freezer-bag','trash-bag','nylex','nylon','packaging','tablecloth'][$i],($i+1)*10]);$ids[]=one('SELECT id FROM ns_categories WHERE name=?',[$name])['id'];}
