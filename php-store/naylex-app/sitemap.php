...
/** @param array<int,array{path:string,lastmod?:?string,image?:?string,image_title?:?string,changefreq?:?string,priority?:?float}> $entries */
function sitemap_urlset(array $entries): string {
 ...
 foreach($entries as $entry){
  ...
  if(!empty($entry['changefreq']))$xml.='<changefreq>'.sitemap_escape($entry['changefreq']).'</changefreq>';
  if(isset($entry['priority']))$xml.='<priority>'.number_format((float)$entry['priority'],1,'.','').'</priority>';
...
function sitemap_static_entries(): array {
 return [
  ...
  ['path'=>'/payment-guide','changefreq'=>'monthly','priority'=>0.4],
