...
function admin_support_panel(): void {
 ...
 ?><div class="support-admin"><aside class="panel support-thread-list"><h2>پیام‌های پشتیبانی</h2><?php if(!$threads):?><p>هنوز پیامی ثبت نشده است.</p><?php endif;foreach($threads as$t):?><a class="<?=$selected===(int)$t['user_id']?'active':''?>" href="/admin?tab=support&amp;user_id=<?=(int)$t['user_id']?>"><strong><?=h($t['name'])?></strong><small><?=h($t['username'])?> · <?=h(substr($t['last_at'],0,16))?></small><?php if((int)$t['unread']):?><b><?=money($t['unread'])?></b><?php endif;?></a><?php endforeach;?></aside><section class="panel support-chat"><?php if(!$customer):?><h2>گفت‌وگویی انتخاب نشده است</h2><?php else:?><div class="section-heading"><div><h2><?=h($customer['name'])?></h2><p class="muted"><?=h($customer['username'])?></p></div><?php action_start('delete_support_thread','','/admin');hidden('id',$customer['id']);?><button class="btn outline" data-confirm="همه پیام‌های این گفت‌وگو حذف شود؟">حذف گفت‌وگو</button></form></div><div class="chat-thread"><?php foreach($messages as$m):?><article class="chat-message <?=$m['sender_role']==='ADMIN'?'from-admin':'from-user'?>"><strong><?=$m['sender_role']==='ADMIN'?'مدیر':'مشتری'?></strong><p><?=nl2br(h($m['message']))?></p><time><?=h(substr($m['created_at'],0,16))?></time></article><?php endforeach;?></div><?php action_start('support_message','chat-form','/admin');hidden('user_id',$customer['id']);?><textarea name="message" rows="3" maxlength="1500" required placeholder="پاسخ مدیر..."></textarea><button class="btn">ارسال پاسخ</button></form><?php endif;?></section></div><?php
}
