...
with sync_playwright() as p:
 ...
 def fill(number):
  page.locator('[name=name]').fill(name);page.locator('[name=username]').fill(number);page.locator('[name=password]').fill('a123');page.get_by_role('button',name='ساخت حساب کاربری',exact=True).click()
 ...
 page.goto(base+'/login');page.locator('[name=username]').fill(phone);page.locator('[name=password]').fill('a123');page.get_by_role('button',name='ورود به حساب',exact=True).click();expect(page).to_have_url(base+'/account')
 print('PASS sticky header, single registration entry, cart +/-, 4-character password, duplicate warning and login')
 browser.close()
